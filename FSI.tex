...
\item[5.] The difference in volume fraction from step 4 gives the $\dot{m}$
   expansion source term to be applied during the pressure update step.
\item[6.] The $\dot{m}$ terms are distributed to one side of the interface.
   If distributed to the destination material side, then distribute where
   $F_{dest}>1/2$.  This guarantees that all faces with at least one 
   adjoining source material cell with have the source material velocity
   defined at that face.  Please refer to the diagram.
\end{description}
...
Please refer to the diagram.  Also, in words: 
\begin{description}
