...
\item[1.] For each cell in a narrow band about the interface, use the
  ...
  interface.  Approximate $\dot{m}$ at the closest point.
\item[2.] extend $\dot{m}$ further into bulk regions.
\item[3.] 
  If distributing $\dot{m}$ to the destination material side, then
...
\item[4.] Using step 3 as a guide, do unsplit CISL phase change advection.
  e.g. if distributing $\dot{m}$ to the destination material side, the 
...
there is a problem.
Suppose evaporation with $\dot{m}$ distributed to the vapor (destination).
Please refer to Figure \ref{mdotproblem}.  Also, in words: 
\begin{description}
\item[1.] During the predictor step, there is no problem because the advection
velocity at time $t^{n}$ is used and all $t^{n}$
source (liquid) material faces
have the source material velocity.
\item[2.] During the corrector step, there is a problem because the advection 
velocity at time $t^{n+1}$ is used which has $\dot{m}$ distributed
to the vapor side of the $t^{n+1}$ interface. But cells on the vapor side at
$t^{n+1}$ might have been liquid cells at $t^{n}$! Now, the discretization
is inconsistent because the interface is not wholly advected by the source
(liquid) velocity.
\end{description}
...
\node[draw] at (2,-1.0) {Gas (destination)};
...
\draw[thick,black,dashed] (0,1.2) -- (4.0,5.2);
...
\foreach \x in {0,...,3}
 \foreach \y in {0,1,2,3}
 {
  ...
  \pgfmathsetmacro{\ytest}{\x+0.7-\y};
  \draw[thick,black,-{>[scale=2.0,black]}] (\x+0.5,\y+0.5) -- (\xx,\yy);
 }
\foreach \y in {0,1}
 \foreach \x in {0,...,3}
 {
  \node at (\x+0.5,\y+0.2) {$\dot{m}$};
 }
\foreach \y in {2,3}
 \foreach \x in {2,3}
 {
...
\node at (1.5,2.2) {$\dot{m}$};
\end{tikzpicture}
\caption{Illustration of $\dot{m}$ redistribution and the problems if 
  predictor corrector method \label{mdotproblem} }
\end{figure}
